...
**Cap 512 oszczędza tylko 60 MB** — czyli tekstury Gothica 2 są w większości ≤512 px i cap 512
...
trzyma je skompresowane **w pełnej rozdzielczości** (~1,38 GB → ~350 MB).
...
Faza 1 istnieje po to, by **cztery niezależne ryzyka zabójcze** sprawdzić w ~2 cyklach CI, zanim
...
2. `cap=512` — miał być tanim wyjściem, oszczędza 60 MB → obalił **moją własną** propozycję i
...
`[swapdiag]`, `[astcdiag]` + obejście `-nomenu`. Wszystkie z komentarzem „TEMP … revert once …".
