...
2. `./ios/build-ios.sh` then `open build-ios/OpenGothic.xcodeproj`.
...
The key is not placed in `Info.plist.in`; maintainers can opt into the separate
entitlements file only for a signing profile that actually grants the capability.
